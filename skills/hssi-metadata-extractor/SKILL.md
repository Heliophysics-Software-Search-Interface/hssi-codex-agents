...
- **Tier A, always** — numpy, scipy, pandas, matplotlib, requests, python-dateutil, pytest, tqdm, PyYAML, click, setuptools and the rest of the generic scientific-Python/tooling stack. Being a dependency is not interoperability; "it directly depends on numpy" is true of nearly every package in HSSI;
...
2. **Every row is SPASE-backed** — each `identifier` is a `https://spase-metadata.org/...` URL, so the whole vocabulary is canonical. You can keep `identifier.startswith("https://spase-metadata.org/")` as a cheap sanity guard, but it no longer excludes anything: there are no non-SPASE rows left to filter out.
...
7. **If you cannot land on exactly one row with an identifier** — whether because several SPASE candidates remain (e.g. `Solar Ultraviolet Imager` → four GOES-16/17/18/19 rows) **or** because no row matches exactly but the vocab still has a plausible same-type row for that name (case-insensitive/trimmed, or an obvious parenthetical-abbreviation variant like `ACE (Advanced Composition Explorer)` vs `ACE`) — do **not** record it as a normal Field 31/32 value. A bare name would silently bind to an arbitrary same-name row or create a near-duplicate of the existing one (the backend matches `filter(name=…, type=…).first()` case-sensitively over the whole table). Instead record it under an explicit **`NEEDS MANUAL RESOLUTION (ambiguous instrument/observatory)`** note for that field — listing the candidate SPASE identifiers — so the validator/submitter treat it as **non-submittable**, not as a clean value.
8. Only when **no row** plausibly matches that `name`+`type` is it safe to record the plain `name` with no identifier as the field value (a genuinely new entry) — still note it for the user rather than implying it's resolved.
