...
6. **Respect field-specific source priority.** Do not use one global source order. Primary repository files and structured citation metadata should beat SoMEF for fields they directly define; PyHC should be prioritized when the package identity match is strong; DataCite/Zenodo are authoritative for DOI-hosted metadata after DOI roles are classified. Treat SoMEF as candidate/corroborating evidence, not as authoritative over repository files or curated metadata.
