...
Always pass the full context the agent needs (repo path, resolved HSSI UUID, target URL, mode, metadata file path, user decisions, validation report, and payload/update-plan paths as applicable).
...
4. If decisions are required, get the user's per-field choices and invoke PREPARE again. If the Updater reconciles a metadata file, invoke **validator** for a focused recheck of its changed fields, then invoke PREPARE with the passing report to produce the final plan.
...
3. Check whether a canonical `hssi_metadata.md` already exists in `hssi-codex-agents/repos/<repo>/`.
4. Ensure the source repo is present and fresh (clone to `repos/` if needed; `git pull`).
5. Invoke **extractor** with the UUID, current HSSI metadata, source repo, and existing canonical file if present. The result must be one complete `hssi_metadata.md` whose provenance header records the UUID, repository URL, full source revision, extraction date, and pending validation state.
6. Invoke **validator** for a full validation of that file. Fix ERRORs through the appropriate agent and revalidate; surface WARNINGs/SUGGESTIONs. Keep the file's validation header pending until the user's final choices are incorporated.
7. Invoke **updater** in **`apply` mode**, PREPARE with the resolved UUID, validated metadata file, passing full validation report, target URL, and update-plan output path. It compares Fields 2–33 to live HSSI without re-extraction and returns a diff plus either an exact update plan or decisions/blockers.
8. Resolve any choices, then finalize. **The finalization sequence is unconditional** — it runs on every full refresh, including a clean no-op where PREPARE reported nothing to decide. Never skip this step for lack of user decisions.
   - **If PREPARE reported choices** — CONFLICTs, removals, or anything else — get the user's explicit per-field decision and invoke the Updater in PREPARE again to reconcile the canonical working file to those values.
   - **Once all choices are resolved, including when there were none:** always invoke the **extractor** to perform its **canonical finalization pass** over the file — prose only, no value changes — so any settled decisions read as final rationale and no run mechanics remain.
   - **Always** invoke the Validator for its **whole-document canonical-state check (Phase 5)**, together with a focused value recheck of any fields the reconciliation changed. When nothing changed, Phase 5 plus the structural/header checks is the whole recheck.
...
- If an agent reports a source it could not reach — typically a publisher returning 402/403, which is
  ...
  A browser will often render a page that a plain fetch cannot, and you may have routes the agent
  lacked. Try it yourself, and hand the text back if you succeed
