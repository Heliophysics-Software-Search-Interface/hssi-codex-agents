...
8. For every CONFLICT, removal, or other choice, get the user's explicit per-field decision. Invoke the Updater in PREPARE again to reconcile the canonical working file. Then invoke the Validator for a focused recheck of only the changed fields, and invoke PREPARE once more with the passing final report to build the exact update plan.
